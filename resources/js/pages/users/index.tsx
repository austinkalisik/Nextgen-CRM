            ...
            <div className="grid gap-4 p-4 xl:grid-cols-[1fr_420px]">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Assign admin, staff, and customer login access.</CardDescription>
                    </CardHeader>
                    <CardContent>
                                ...
                                <tbody>
                                    {users.map((user) => (
                                        <tr key={user.id} className="border-b last:border-0">
                                            ...
                                            <td className="py-3">{user.customer?.company_name ?? 'Internal'}</td>
                                            ...
                                            <td className="py-3 text-right">
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    <Button
                                                        size="sm"
                                                        variant="destructive"
                                                        onClick={() => router.delete(`/users/${user.id}`, { preserveScroll: true })}
                                                    >
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Customer users require a linked customer record.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={submit} className="flex flex-col gap-4">
                            ...
                            <Field label="Linked Customer" error={form.errors.customer_id}>
                                <Select value={form.data.customer_id || 'none'} onValueChange={(value) => form.setData('customer_id', value === 'none' ? '' : value)}>
                                    <SelectTrigger className="w-full">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                            ...
                            <Field label={editing ? 'New Password' : 'Password'} error={form.errors.password}>
                                <Input type="password" value={form.data.password} onChange={(e) => form.setData('password', e.target.value)} required={!editing} />
                            </Field>
